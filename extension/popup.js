...
async function refresh() { const values = await chrome.storage.local.get(["publishUrl", "token", "intervalMinutes", "monitorStatus"]); $("url").value = values.publishUrl || defaults.publishUrl; $("token").value = values.token || ""; $("interval").value = String(values.intervalMinutes || 60); const status = values.monitorStatus || { state: "needs_setup", message: "Add settings to begin." }; $("status").textContent = `${status.state.replaceAll("_", " ")}: ${status.message || ""}${status.updatedAt ? `\n${new Date(status.updatedAt).toLocaleString()}` : ""}`; }
