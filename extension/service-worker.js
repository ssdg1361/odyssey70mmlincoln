const DATES = ["2026-08-21","2026-08-22","2026-08-23","2026-08-24","2026-08-25","2026-08-26","2026-08-31","2026-09-01","2026-09-02","2026-09-03","2026-09-04","2026-09-05","2026-09-06","2026-09-07","2026-09-08","2026-09-09","2026-09-10","2026-09-11","2026-09-12","2026-09-13","2026-09-14","2026-09-15","2026-09-16"];
const ALARM = "odyssey-seat-monitor";
const pendingRuns = new Map();

chrome.runtime.onInstalled.addListener(async () => { const { intervalMinutes } = await chrome.storage.local.get("intervalMinutes"); await configureAlarm(intervalMinutes || 60); await setStatus({ state: "needs_setup", message: "Add the dashboard publishing token, then start monitoring." }); });
chrome.alarms.onAlarm.addListener((alarm) => { if (alarm.name === ALARM) runMonitor(); });
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "amc_collection_result") { const pending = pendingRuns.get(message.runId); if (pending && sender.tab?.id === pending.tabId) pending.resolve(message); return; }
  if (message.type === "save_settings") { chrome.storage.local.set(message.settings).then(async () => { await configureAlarm(message.settings.intervalMinutes); await setStatus({ state: "ready", message: "Monitor scheduled." }); sendResponse({ ok: true }); }); return true; }
...
function waitForTab(tabId) { return new Promise((resolve, reject) => { const timeout = setTimeout(() => { chrome.tabs.onUpdated.removeListener(listener); reject(new Error("AMC page did not finish loading.")); }, 30_000); const listener = (id, info) => { if (id === tabId && info.status === "complete") { clearTimeout(timeout); chrome.tabs.onUpdated.removeListener(listener); resolve(); } }; chrome.tabs.onUpdated.addListener(listener); }); }
function waitForCollection(runId, tabId) { return new Promise((resolve, reject) => { const timeout = setTimeout(() => { pendingRuns.delete(runId); reject(new Error("AMC collection timed out before returning a result.")); }, 180_000); pendingRuns.set(runId, { tabId, resolve: (value) => { clearTimeout(timeout); pendingRuns.delete(runId); resolve(value); } }); }); }
...
async function runMonitor() {
  ...
  await setStatus({ state: "running", message: "Checking AMC in a temporary browser tab…" });
  ...
  try {
    ...
    const collected = waitForCollection(runId, tab.id);
    await chrome.scripting.executeScript({ target: { tabId: tab.id }, world: "MAIN", func: collectFromAmcPage, args: [DATES, runId] });
    const outcome = await collected;
    if (!outcome.ok) throw new Error(outcome.message || "AMC did not return a collection result.");
    const response = await fetch(publishUrl, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify(outcome.snapshot) });
    const published = await response.json();
    if (!response.ok) throw new Error(published.message || "Dashboard rejected the snapshot.");
    await setStatus({ state: "ok", message: `Published ${published.performances} performance${published.performances === 1 ? "" : "s"}.`, checkedAt: outcome.snapshot.checkedAt });
  } catch (error) {
...
async function collectFromAmcPage(dates, runId) {
  const send = (data) => window.postMessage({ source: "odyssey-seat-monitor", runId, ...data }, location.origin);
  ...
  const fetchPage = async (path) => { const response = await fetch(path, { credentials: "same-origin" }); const text = await response.text(); if (!response.ok) throw new Error(`AMC request failed (${response.status}).`); if (blocked(text)) throw new Error("AMC_ACCESS_BLOCKED: AMC returned an access-control page."); return text; };
  const idsOnListing = (html) => { const doc = new DOMParser().parseFromString(html, "text/html"); const ids = new Set(); for (const link of doc.querySelectorAll('a[href*="/showtimes/"]')) { const group = link.closest('li[role="listitem"][aria-label]'); const text = `${group?.getAttribute("aria-label") || ""} ${group?.textContent || ""}`; const id = link.getAttribute("href")?.match(/\/showtimes\/(\d+)(?:\/seats)?(?:[?#]|$)/)?.[1]; if (id && /IMAX\s*70\s*MM/i.test(text)) ids.add(id); } return [...ids]; };
